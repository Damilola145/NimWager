...
import { createGame, fundGame } from "@/api/games"
...
export function CreateWagerModal({
  ...
  async function handleConfirm() {
    ...
    try {
      const signature = await signChallenge(buildChallenge("create", session.address))
      const creatorSeedCommitment = await sha256Hex(seed)
      setPhase("submitting")
      ...
      rememberGameSecret(res.gameId, seed)
      await fundGame(res.gameId, { playerAddress: session.address, htlcTxHash: `mock_htlc_${res.gameId}`, htlcContractAddress: `mock_contract_${res.gameId}`, preImageHash: await sha256Hex(seed), signature, publicKey: session.address })
